...
def load_dataset(name: str, folder: Union[Path, str] = dataset_dir):
    ...
    def get_inter_times(seq: dict):
        ...
        #print(f'inter times: {np.ediff1d(np.concatenate([[seq["t_start"]], seq["arrival_times"], [seq["t_end"]]]))}')
        return np.ediff1d(np.concatenate([[seq["t_start"]], seq["arrival_times"], [seq["t_end"]]]))

    sequences = [
        Sequence(
            inter_times=get_inter_times(seq),
            src_marks=seq.get("src_marks"),
            dst_marks=seq.get("dst_marks"),
            meta=seq.get("meta"),
    ...
    return SequenceDataset(sequences=sequences, num_src_marks=dataset.get("num_src_marks", 1), num_dst_marks=dataset.get("num_dst_marks", 1), num_meta_classes=dataset.get("num_meta_classes", 0))


class SequenceDataset(torch.utils.data.Dataset):
    def __init__(self, sequences: List[Sequence], num_src_marks=1, num_dst_marks=1, num_meta_classes=0):
    ...
    def __add__(self, other: "SequenceDataset") -> "SequenceDataset":
        ...
        new_num_src_marks = max(self.num_src_marks, other.num_src_marks)
        new_num_dst_marks = max(self.num_dst_marks, other.num_dst_marks)
        new_sequences = self.sequences + other.sequences
        return SequenceDataset(new_sequences, num_src_marks=new_num_src_marks, num_dst_marks=new_num_dst_marks)
    ...
    def train_val_test_split(
            self, train_size=0.6, val_size=0.2, test_size=0.2, single_sequence=False, seed=None, shuffle=True,
        ...
        return (
            SequenceDataset(train_sequences, num_src_marks=self.num_src_marks, num_dst_marks=self.num_dst_marks, num_meta_classes=self.num_meta_classes),
            SequenceDataset(val_sequences, num_src_marks=self.num_src_marks, num_dst_marks=self.num_dst_marks, num_meta_classes=self.num_meta_classes),
            SequenceDataset(test_sequences, num_src_marks=self.num_src_marks, num_dst_marks=self.num_dst_marks, num_meta_classes=self.num_meta_classes),
        )
